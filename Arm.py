...
while True:
    ...
    cv2.line(frame, center_of_screen, (frame_width, frame_height // 2), (0, 0, 255), 2)
    ...
    if len(faces) > 0:
        ...
        angle_between_lines = int(calculate_angle(center_of_screen, (frame_width, frame_height // 2), center_of_screen, center_of_face))
        # screenx,_ = center_of_screen
        # facex,_ = center_of_face
        # if screenx > facex:
        #     angle_between_lines += 90
        print(f"Angle: {angle_between_lines} degrees")
        send_angle(angle_between_lines)
        time.sleep(0.05)
